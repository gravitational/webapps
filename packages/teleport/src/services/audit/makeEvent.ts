...
export const formatters: Formatters = {
  ...
  [eventCodes.EXEC_FAILURE]: {
    ...
    format: ({ user, exitError, ...rest }) =>
      `User [${user}] command execution on node ${rest['server_hostname']||rest['addr.local']} failed [${exitError}]`,
  },
  ...
  [eventCodes.SCP_DOWNLOAD_FAILURE]: {
    ...
    format: ({ exitError, ...rest }) =>
      `File download from node [${rest['server_hostname']||rest['addr.local']}] failed [${exitError}]`,
  },
  [eventCodes.SCP_UPLOAD]: {
    type: 'scp',
    desc: 'SCP Upload',
    format: ({ user, path, ...rest }) =>
      `User [${user}] uploaded a file [${path}] to node [${rest['server_hostname']||rest['addr.local']}]`,
  },
  [eventCodes.SCP_UPLOAD_FAILURE]: {
    type: 'scp',
    desc: 'SCP Upload Failed',
    format: ({ exitError, ...rest }) =>
      `File upload to node [${rest['server_hostname']||rest['addr.local']}] failed [${exitError}]`,
  },
  ...
  [eventCodes.SFTP_OPEN_FAILURE]: {
    ...
    format: ({ user, path, error, ...rest }) =>
      `User [${user}] failed to open file [${path}] on node [${rest['server_hostname']||rest['addr.local']}]: [${error}]`,
  },
  [eventCodes.SFTP_CLOSE]: {
    type: 'sftp',
    desc: 'SFTP Close',
    format: ({ user, path, ...rest }) =>
      `User [${user}] closed file [${path}] on node [${rest['server_hostname']||rest['addr.local']}]`,
  },
  [eventCodes.SFTP_CLOSE_FAILURE]: {
    type: 'sftp',
    desc: 'SFTP Close Failed',
    format: ({ user, path, error, ...rest }) =>
      `User [${user}] failed to close file [${path}] on node [${rest['server_hostname']||rest['addr.local']}]: [${error}]`,
  },
  [eventCodes.SFTP_READ]: {
    type: 'sftp',
    desc: 'SFTP Read',
    format: ({ user, path, ...rest }) =>
      `User [${user}] read from file [${path}] on node [${rest['server_hostname']||rest['addr.local']}]`,
  },
  [eventCodes.SFTP_READ_FAILURE]: {
    type: 'sftp',
    desc: 'SFTP Read Failed',
    format: ({ user, path, error, ...rest }) =>
      `User [${user}] failed to read from file [${path}] on node [${rest['server_hostname']||rest['addr.local']}]: [${error}]`,
  },
  [eventCodes.SFTP_WRITE]: {
    type: 'sftp',
    desc: 'SFTP Write',
    format: ({ user, path, ...rest }) =>
      `User [${user}] wrote to file [${path}] on node [${rest['server_hostname']||rest['addr.local']}]`,
  },
  [eventCodes.SFTP_WRITE_FAILURE]: {
    type: 'sftp',
    desc: 'SFTP Write Failed',
    format: ({ user, path, error, ...rest }) =>
      `User [${user}] failed to write to file [${path}] on node [${rest['server_hostname']||rest['addr.local']}]: [${error}]`,
  },
  [eventCodes.SFTP_LSTAT]: {
    type: 'sftp',
    desc: 'SFTP Lstat',
    format: ({ user, path, ...rest }) =>
      `User [${user}] queried attributes of file [${path}] on node [${rest['server_hostname']||rest['addr.local']}]`,
  },
  [eventCodes.SFTP_LSTAT_FAILURE]: {
    type: 'sftp',
    desc: 'SFTP Lstat Failed',
    format: ({ user, path, error, ...rest }) =>
      `User [${user}] failed to query attributes of file [${path}] on node [${rest['server_hostname']||rest['addr.local']}]: [${error}]`,
  },
  ...
  [eventCodes.SFTP_SETSTAT_FAILURE]: {
    ...
    format: ({ user, path, error, ...rest }) =>
      `User [${user}] failed to change attributes of file [${path}] on node [${rest['server_hostname']||rest['addr.local']}]: [${error}]`,
  },
  ...
  [eventCodes.SFTP_OPENDIR_FAILURE]: {
    ...
    format: ({ user, path, error, ...rest }) =>
      `User [${user}] failed to open directory [${path}] on node [${rest['server_hostname']||rest['addr.local']}]: [${error}]`,
  },
  ...
  [eventCodes.SFTP_READDIR_FAILURE]: {
    ...
    format: ({ user, path, error, ...rest }) =>
      `User [${user}] failed to read directory [${path}] on node [${rest['server_hostname']||rest['addr.local']}]: [${error}]`,
  },
  ...
  [eventCodes.SFTP_REMOVE_FAILURE]: {
    ...
    format: ({ user, path, error, ...rest }) =>
      `User [${user}] failed to remove file [${path}] on node [${rest['server_hostname']||rest['addr.local']}]: [${error}]`,
  },
  ...
  [eventCodes.SFTP_MKDIR_FAILURE]: {
    ...
    format: ({ user, path, error, ...rest }) =>
      `User [${user}] failed to create directory [${path}] on node [${rest['server_hostname']||rest['addr.local']}]: [${error}]`,
  },
  ...
  [eventCodes.SFTP_RMDIR_FAILURE]: {
    ...
    format: ({ user, path, error, ...rest }) =>
      `User [${user}] failed to remove directory [${path}] on node [${rest['server_hostname']||rest['addr.local']}]: [${error}]`,
  },
  [eventCodes.SFTP_REALPATH]: {
    type: 'sftp',
    desc: 'SFTP Realpath',
    format: ({ user, path, ...rest }) =>
      `User [${user}] queried absolute path of file [${path}] on node [${rest['server_hostname']||rest['addr.local']}]`,
  },
  [eventCodes.SFTP_REALPATH_FAILURE]: {
    type: 'sftp',
    desc: 'SFTP Realpath Failed',
    format: ({ user, path, error, ...rest }) =>
      `User [${user}] failed to query absolute path of file [${path}] on node [${rest['server_hostname']||rest['addr.local']}]: [${error}]`,
  },
  ...
  [eventCodes.SFTP_RENAME_FAILURE]: {
    ...
    format: ({ user, path, error, ...rest }) =>
      `User [${user}] failed to rename file [${path}] on node [${rest['server_hostname']||rest['addr.local']}]: [${error}]`,
  },
  [eventCodes.SFTP_READLINK]: {
    type: 'sftp',
    desc: 'SFTP Readlink',
    format: ({ user, path, ...rest }) =>
      `User [${user}] read symbolic link [${path}] on node [${rest['server_hostname']||rest['addr.local']}]`,
  },
  [eventCodes.SFTP_READLINK_FAILURE]: {
    type: 'sftp',
    desc: 'SFTP Readlink Failed',
    format: ({ user, path, error, ...rest }) =>
      `User [${user}] failed to read symbolic link [${path}] on node [${rest['server_hostname']||rest['addr.local']}]: [${error}]`,
  },
  ...
  [eventCodes.SFTP_SYMLINK_FAILURE]: {
    ...
    format: ({ user, path, error, ...rest }) =>
      `User [${user}] failed to create symbolic link [${path}] on node [${rest['server_hostname']||rest['addr.local']}]: [${error}]`,
  },
